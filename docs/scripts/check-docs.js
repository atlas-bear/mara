...
const missingMainDocs = checkFilesExist(mainDocIds, path.join(__dirname, '../docs'));
const missingApiDocs = checkFilesExist(apiDocIds, path.join(__dirname, '../api'));
  ...
  process.stdin.once('data', (input) => {
    ...
    if (answer === 'y' || answer === 'yes') {
      ...
      function generateStubFile(docId, filePath) {
        const title = docId.split('/').pop().replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
        const content = `---
id: ${docId.split('/').pop()}
title: ${title}
sidebar_label: ${title}
