 ...
 * 
 * This on-demand function serves as a fallback for the scheduled background generation.
 * It first checks for cached content (generated by the background function), and only
 * calls the LLM service if no cached content is available.
 * 
 * NOTE: This function should rarely need to generate content, as the background function
 * is scheduled to run once a week at the end of the reporting period.
 */
...
export const handler = async (event) => {
  ...
  try {
    ...
    if (cachedContent) {
      log.info(`Using cached weekly report content for ${start} to ${end}`);
      ...
      return {
        ...
        headers: {
          ...
          "X-Cache": "HIT",
          "X-Cache-Source": "weekly-background"
        },
        body: JSON.stringify(cachedContent)
      };
    }
    
    log.info(`No cached content found for ${start} to ${end}, generating new content (fallback mode)`);
    ...
    try {
      ...
      // Call Claude to generate the weekly report content (fallback mode)
      log.info("Calling Claude to generate weekly report content (fallback mode)");
      const reportContent = await callClaudeWithPrompt("weeklyReport", reportData);
      
      // Cache the generated content with the weekly cache (7-day TTL)
      await weeklyReportCache.store(cacheKey, reportContent);
      log.info(`Cached new weekly report content for ${start} to ${end}`);
      ...
      return {
        ...
        headers: {
          ...
          "Cache-Control": "public, max-age=86400", // Cache for 24 hours in browser
          "X-Cache": "MISS",
          "X-Cache-Source": "on-demand-fallback"
        },
