...
import {
  ...
  getVesselByName,
  getVesselById,
} from "./utils/vessel-utils.js";
import { validateData } from "./utils/validation.js";
import { corsHeaders } from "./utils/environment.js";
import {
  generateFlashReportToken,
  getPublicFlashReportUrl,
...
export const handler = async (event, context) => {
  ...
  try {
    ...
    console.log(
      "SendGrid API configured with key length:",
      apiKey ? apiKey.length : "not found"
    );
    ...
    const sampleIncidentData = {
      ...
      description:
        "Test incident description for flash report from serverless function.",
      response_type: ["Action 1", "Action 2"],
      authorities_notified: ["Local Maritime Authority"],
      items_stolen: ["Ship supplies"],
      analysis:
        "This is a sample analysis for testing the flash report email functionality.",
      ...
      if (!incidentData) {
        ...
        try {
          ...
          if (cachedData) {
            ...
            console.log(
              "DEBUG - Cached data content:",
              JSON.stringify(cachedData).substring(0, 500)
            );

            // Enhanced debugging for vessel data in the new comprehensive structure
            console.log("VESSEL DATA STRUCTURE CHECK:");
            console.log(
              "- Has vessels_involved array:",
              !!cachedData.vessels_involved &&
                Array.isArray(cachedData.vessels_involved)
            );

            if (
              cachedData.vessels_involved &&
              cachedData.vessels_involved.length > 0
            ) {
              console.log(
                "- Number of vessels:",
                cachedData.vessels_involved.length
              );
              console.log(
                "- First vessel data:",
                JSON.stringify(cachedData.vessels_involved[0])
              );
            } else {
            ...
            if (!cachedData.id || !cachedData.title) {
              console.error(
                "Invalid or empty cached data structure, forcing refresh..."
              );

              // Force a refresh to get fresh data
              const refreshedData = await getCachedIncident(incidentId, {
                forceRefresh: true,
              });
              if (!refreshedData || !refreshedData.id) {
                console.error(
                  "Still unable to get valid data after force refresh"
                );
            ...
            if (
              cachedData.incident_type &&
              cachedData.incident_type.length > 0
            ) {
            ...
            if (
              ...
              console.log(
                "Extracted vessel data from vessels_involved array:",
            ...
            console.log(
              "No data found in cache, falling back to direct Airtable fetch"
            );

            // Fall back to direct Airtable fetch if cache fails
            try {
              console.log(
                "Attempting to fetch incident data directly from Airtable..."
              );
              ...
              if (airtableData) {
                ...
                incidentData =
                  airtableData.incident?.fields || airtableData.incident || {};

                // CRITICAL: Make sure we get vessel fields properly
                if (airtableData.vessel) {
                  console.log(
                    "DEBUG: Direct Airtable vessel data:",
                    JSON.stringify(airtableData.vessel).substring(0, 200)
                  );
                  vesselData =
                    airtableData.vessel.fields || airtableData.vessel || {};
                  console.log(
                    "DEBUG: Extracted vessel data:",
                    JSON.stringify(vesselData)
                  );
                } else {
                ...
                const incidentVesselData =
                  airtableData.incidentVessel?.fields ||
                  airtableData.incidentVessel ||
                  {};
                const incidentTypeData =
                  airtableData.incidentType?.fields ||
                  airtableData.incidentType ||
                  {};
                ...
                if (incidentVesselData) {
                  if (incidentVesselData.vessel_status_during_incident) {
                    incidentData.vessel_status_during_incident =
                      incidentVesselData.vessel_status_during_incident;
                  }
                  ...
                  if (incidentVesselData.damage_sustained) {
                    incidentData.damage_sustained =
                      incidentVesselData.damage_sustained;
                  }
                }
              ...
              console.warn(
                "Both cache and direct Airtable fetch failed:",
          ...
          console.log(
            "Sample incident ID check:",
            incidentId,
            "Matches?",
            incidentId === "2025-0010"
          );
        ...
        console.log(
          "Using client-provided incident data, skipping Airtable fetch"
        );
        ...
        console.log(
          "Created vessel data from client data:",
          JSON.stringify(vesselData)
        );
      }
    ...
    try {
      ...
      if (
        incidentData.map_image_url &&
        typeof incidentData.map_image_url === "string"
      ) {
        mapImageUrl = incidentData.map_image_url;
        console.log("Using map image URL from Airtable:", mapImageUrl);
      } else {
        console.log(
          "No map_image_url found in incident data, using default map"
        );

        // Fall back to a default map in Cloudinary
        mapImageUrl =
          "https://res.cloudinary.com/dwnh4b5sx/image/upload/maps/public/default-map.jpg";
      }
    } catch (mapError) {
      ...
      mapImageUrl =
        "https://res.cloudinary.com/dwnh4b5sx/image/upload/maps/public/error-map.jpg";
    ...
    if (
      incidentData.incident_type_name &&
      typeof incidentData.incident_type_name === "string"
    ) {
    ...
    // Last resort: use any other available fields
      ...
      incidentType =
        typeof incidentData.type === "string" ? incidentData.type : "Incident";
    }
    ...
    console.log(
      "- vesselData keys:",
      vesselData ? Object.keys(vesselData).join(", ") : "none"
    );
    ...
    if (incidentData.title) {
      ...
      for (const indicator of vesselIndicators) {
        ...
        if (index !== -1) {
          ...
          console.log(
            `Extracted vessel name from title using "${indicator}": ${extractedVesselName}`
          );
          break;
        }
      }
    ...
    console.log(
      "- incidentData full object:",
      JSON.stringify(incidentData).substring(0, 500) + "..."
    );
    console.log(
      "- vesselData full object:",
      JSON.stringify(vesselData).substring(0, 500) + "..."
    );
    ...
    if (
      ...
      !enhancedVesselData?.type ||
      !enhancedVesselData?.flag
    ) {
      console.log(
        "Incomplete vessel data detected. Attempting alternative lookups..."
      );

      // Strategy 1: Check for direct vessel reference in the incident
      if (
        incidentData.vessel &&
        typeof incidentData.vessel === "string" &&
        incidentData.vessel.startsWith("rec")
      ) {
        console.log(
          "Found direct vessel reference ID in incident:",
          incidentData.vessel
        );
        try {
          ...
          if (directVesselData) {
            console.log(
              "Successfully fetched vessel by direct ID reference:",
              Object.keys(directVesselData).join(", ")
            );
          ...
          console.error(
            "Error looking up vessel by ID:",
            vesselIdError.message
          );
        }
      }
      ...
      if (
        (!enhancedVesselData?.type || !enhancedVesselData?.flag) &&
        ...
        try {
          ...
          if (imoVesselData) {
            console.log(
              "Successfully fetched vessel by IMO:",
              Object.keys(imoVesselData).join(", ")
            );
      ...
      if (
        ...
        if (foundName) {
          console.log(
            `Found vessel name "${foundName}", attempting name lookup...`
          );
          try {
            const nameVesselData = await getVesselByName(foundName);
            if (nameVesselData) {
              console.log(
                "Successfully fetched vessel by name:",
                Object.keys(nameVesselData).join(", ")
              );
            ...
            console.error(
              "Error looking up vessel by name:",
              nameError.message
            );
          }
        }
    ...
    console.log(
      "Final enhanced vessel data:",
      JSON.stringify(enhancedVesselData)
    );
    ...
    const vesselName =
      enhancedVesselData?.name || extractedVesselName || "Unknown Vessel";
    ...
    const preparedIncident = {
      ...
      location:
        incidentData.location_name ||
        incidentData.location ||
        "Unknown Location",
      coordinates: {
        latitude:
          latitude !== null && !isNaN(latitude) ? parseFloat(latitude) : 0,
        longitude:
          longitude !== null && !isNaN(longitude) ? parseFloat(longitude) : 0,
      },
      ...
      destination:
        incidentData.vessel_destination ||
        incidentData.destination ||
        "Unknown Destination",

      // Use crew_impact from incidentData, falling back to crewStatus (client-side name)
      crew_impact:
        incidentData.crew_impact ||
        incidentData.crewStatus ||
    ...
    if (testMode) {
      ...
      console.log(
        `TEST MODE ENABLED: Will generate tokens and URLs${skipEmails ? " but skip actual email sending" : " and send actual emails"}`
      );
      ...
      const testResults = await Promise.all(
        recipients.map(async (recipient) => {
          ...
          const brandParam = shouldUseClientBranding(recipient.email)
            ? "client"
            : null;

          // Generate public flash report URL
          const publicUrl = getPublicFlashReportUrl(
            incidentId,
            tokenData.token,
            brandParam
          );
      ...
      console.log(
        "TEST MODE: Continuing to send actual emails with pre-generated tokens"
      );
    }
    ...
    if (!sendGridApiKey) {
      ...
      console.log(
        "TESTING MODE: Would send emails to:",
        recipients.map((r) => r.email).join(", ")
      );
      console.log(
        "Public URLs generated:",
        testResults.map((r) => `${r.email}: ${r.publicUrl}`).join("\n")
      );
      ...
      return {
        ...
        body: JSON.stringify({
          message:
            "TESTING MODE: Email would be sent (no SendGrid API key provided)",
    ...
    const results = await Promise.all(
      recipients.map(async (recipient) => {
        try {
          ...
          console.log(
            "- incidentData has keys:",
            incidentData ? Object.keys(incidentData).join(", ") : "no data"
          );
          console.log(
            "- Does client directly provide incident?",
            !!payload.incident
          );
          ...
          if (payload.incident) {
            ...
            console.log(
              "CLIENT PROVIDED VESSEL DATA:",
              JSON.stringify(clientVesselData)
            );
            ...
            if (incidentData.vessel_status_during_incident) {
              incidentVesselFields.vessel_status_during_incident =
                incidentData.vessel_status_during_incident;
            }
            ...
            if (
              (!vesselData || Object.keys(vesselData).length === 0) &&
              enhancedVesselData &&
              Object.keys(enhancedVesselData).length > 0
            ) {
              console.log(
                "⚠️ Using enhanced vessel data instead of empty vesselData"
              );
            ...
            // This ensures we always have vessel data in the email
              ...
              console.log(
                "⚠️ vesselData missing or empty, using prepared data instead"
              );
              vesselData = {
            ...
            console.log(
              "- Final crew status:",
              incidentVesselFields.crew_impact
            );
            ...
            templateData = {
              incident: {
                ...
                vessels_involved: [
                  {
                    // Add vessel data in the vessels_involved format
            ...
            console.log(
              "USING COMPREHENSIVE DATA STRUCTURE FOR EMAIL TEMPLATE:"
            );
            console.log("- vessel name:", templateData.incident.vesselName);
          ...
          if (isFlat) {
            ...
            console.log(
              "- vessel data (nested):",
              JSON.stringify(templateData.incident.vessel?.fields || {})
            );
            console.log(
              "- vessel name:",
              templateData.incident.vessel?.fields?.name
            );
            console.log(
              "- vessel type:",
              templateData.incident.vessel?.fields?.type
            );
            console.log(
              "- vessel flag:",
              templateData.incident.vessel?.fields?.flag
            );
          }
          ...
          if (isFlat) {
            ...
            console.log(
              "- Vessel name (flat):",
              templateData.incident.vesselName
            );
          } else {
            console.log(
              "- Nested template vessel data:",
              JSON.stringify(templateData.incident.vessel?.fields || {})
            );
            console.log(
              "- Vessel name (nested):",
          ...
          console.log(
            "- preparedIncident vessel name:",
            preparedIncident.vesselName
          );
          ...
          try {
            ...
            console.log(
              `Attempting to send email to ${recipient.email} with SendGrid...`
            );
            console.log(
              `From: ${sendGridEmailData.from.email} (${sendGridEmailData.from.name})`
            );
            ...
            console.error(
              `SendGrid error for ${recipient.email}:`,
              sendGridError
            );
          ...
          try {
            ...
            console.error(
              `Error generating fallback URL for ${recipient.email}:`,
              urlError
            );
            return {
              email: recipient.email,
...
function getDefaultBranding() {
  ...
  const envVars = Object.keys(process.env)
    .filter(
      (key) =>
        key.includes("LOGO") || key.includes("COMPANY") || key.includes("COLOR")
    )
    .filter((key) => !key.includes("SECRET"));
