...
export async function handler(event) {
  try {
    ...
    const isManualTest = event.httpMethod === "POST" && event.body;
    ...
    // Parse request body for manual test parameters
    let testMode = false;
    let testEmail = null;
    ...
    console.log(`Weekly report notification function triggered: ${isScheduled ? "Scheduled" : "Manual"}`);
    ...
    if (testMode) {
      // In test mode, we'll only send to the test email if provided
      if (testEmail) {
        // Create a test recipient with the provided email
        recipients = [{
          email: testEmail,
          name: 'Test User',
        ...
        if (includeClientExample) {
          ...
          if (clientDomains.length > 0) {
            ...
            skippedRecipients = allRecipients.slice(recipients.length);
          }
        }
      } else {
        console.log("Test mode enabled but no test email provided");
      }
    } else {
      // Normal mode - get all recipients who have opted in for weekly reports
      recipients = await getWeeklyReportRecipients();
    ...
    console.log(`Found ${recipients.length} recipients for weekly report notification${testMode ? ' (test mode)' : ''}`);
    ...
    // For test mode with client example, prepare the skipped emails info
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        results,
        skippedEmailsInfo: testMode ? skippedEmailsInfo : undefined,
      }),
    };
